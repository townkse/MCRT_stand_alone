...
FILE * fp = fopen("sigmadiff_ionisation_e_emfietzoglou.dat","r");
...
while (1) 
   {
	 ...
	 // This is different from the proton macro case for which the max* are ONLY for the secondary kinetic
         ...
         if ((e+32.2)/2. > e) max4=e;
         else max4 = (e+32.2)/2.;
	 
         if ((e+539.7)/2. > e) max5=e;
         else max5 = (e+539.7)/2.;
	 
	 if (trans>=10.&&trans<=max1) d1cum[comptbis]=d1cum[comptbis]+(all_xs1[myCompt]+all_xs1[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=13.&&trans<=max2) d2cum[comptbis]=d2cum[comptbis]+(all_xs2[myCompt]+all_xs2[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=17.&&trans<=max3) d3cum[comptbis]=d3cum[comptbis]+(all_xs3[myCompt]+all_xs3[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=32.2&&trans<=max4) d4cum[comptbis]=d4cum[comptbis]+(all_xs4[myCompt]+all_xs4[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=539.7&&trans<=max5) d5cum[comptbis]=d5cum[comptbis]+(all_xs5[myCompt]+all_xs5[myCompt-1])*0.5*width_e[myCompt];
	...
	comptbis++;
         ...
         if (trans>=10.&&trans<=max1) d1cum[comptbis]=(all_xs1[myCompt]+all_xs1[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=13.&&trans<=max2) d2cum[comptbis]=(all_xs2[myCompt]+all_xs2[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=17.&&trans<=max3) d3cum[comptbis]=(all_xs3[myCompt]+all_xs3[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=32.2&&trans<=max4) d4cum[comptbis]=(all_xs4[myCompt]+all_xs4[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=539.7&&trans<=max5) d5cum[comptbis]=(all_xs5[myCompt]+all_xs5[myCompt-1])*0.5*width_e[myCompt];
      }
...
remove ("sigmadiff_cumulated_ionisation_e_emfietzoglou.dat");

FILE * fpout = fopen("sigmadiff_cumulated_ionisation_e_emfietzoglou.dat","w");
FILE * fp = fopen("sigmadiff_ionisation_e_emfietzoglou.dat","r");
...
while (1) 
   {
      ...
      if (e==tab_e[comptter])
      {
        ...
        if (trans>=10.&&trans<=max1) sumd1=sumd1+(all_xs1[myCompt]+all_xs1[myCompt-1])*0.5*width_e[myCompt];
        if (trans>=13.&&trans<=max2) sumd2=sumd2+(all_xs2[myCompt]+all_xs2[myCompt-1])*0.5*width_e[myCompt];
        if (trans>=17.&&trans<=max3) sumd3=sumd3+(all_xs3[myCompt]+all_xs3[myCompt-1])*0.5*width_e[myCompt];
        if (trans>=32.2&&trans<=max4) sumd4=sumd4+(all_xs4[myCompt]+all_xs4[myCompt-1])*0.5*width_e[myCompt];
        if (trans>=539.7&&trans<=max5) sumd5=sumd5+(all_xs5[myCompt]+all_xs5[myCompt-1])*0.5*width_e[myCompt];
	...
	comptter++;
        ...
        sumd1=0;sumd2=0;sumd3=0;sumd4=0;sumd5=0;
         ...
         if (trans>=10.&&trans<=max1)  sumd1=(all_xs1[myCompt]+all_xs1[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=13.&&trans<=max2)  sumd2=(all_xs2[myCompt]+all_xs2[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=17.&&trans<=max3)  sumd3=(all_xs3[myCompt]+all_xs3[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=32.2&&trans<=max4)  sumd4=(all_xs4[myCompt]+all_xs4[myCompt-1])*0.5*width_e[myCompt];
         if (trans>=539.7&&trans<=max5)  sumd5=(all_xs5[myCompt]+all_xs5[myCompt-1])*0.5*width_e[myCompt];
